...
nameslist = ['Sample', 'ctDNA fraction', 'Mutation count', 'DNA repair defect', 'BRCA2', 'ATM', 'CDK12','AR', 'AR_muts', 'PTEN', 'PI3K', 'RB1','TP53', 'APC_or_CTNNB1', 'SPOP', 'FOXA1']
...
df_cn = df_cn.drop("AR_muts", axis = 1).reset_index(drop = True)
...
# add mutation and CN information from two genes (MSH2 and 6) - taken from the notes column
row_index = int(np.where(df_cn["Sample"] == "CAVK-0006_Baseline")[0])
df_cn["MSH2"] = np.repeat(0, len(df_cn))
df_cn["MSH2"][row_index] = -1

row_index = int(np.where(df_cn["Sample"] == "CAVA-0008_Baseline")[0])
df_cn["MSH6"] = np.repeat(0, len(df_cn))
df_cn["MSH6"][row_index] = -1
...
df_muts.loc[94, "Effect"] = "Structural rearrangement"
...
if "fs" in df_muts.loc[df_muts["Effect"].astype(str).str.startswith("p."), "Effect"]: 
    df_muts.loc[df_muts["Effect"].astype(str).str.startswith("p."), "Effect"] = "Frameshift mutation"

df_muts.loc[df_muts["Effect"].astype(str).str.startswith("p."), "Effect"] = "Missense mutation"
...
# EXTRA GENES TO ADD MANUALLY
#######################
genes_df = pd.DataFrame([["CAVA-0008_Baseline", "MSH6", "Missense mutation"], ["CAVK-0006_Baseline", "MSH2", "Missense mutation"], ["CAMP-0003_Baseline", "MSH6", "Structural rearrangement"]])
genes_df.columns = ["Sample", "Gene", "Effect"]
df_muts = df_muts.append(genes_df, ignore_index=True)

# ADD PATIENTS WITH 0 ctDNA PERCENTAGE to DF_CN
#######################
PATH_to_df = "/groups/wyattgrp/users/amunzur/ind232/data/IND232_cfDNAresults_Aug2019.csv"
df_main = pd.read_csv(PATH_to_df)
df_main = df_main.loc[df_main['ctDNA detected'] == "no"].reset_index(drop = True) # remove patients that didn't have any ctDNA detected 
df_main['Sample'] = df_main[df_main.columns[[0, 1]]].apply(lambda x: '_'.join(x.astype(str)), axis=1) # merge study id and sample cols into one
del df_main["ctDNA detected"]
del df_main["Study ID"]
df_main = df_main.fillna(0)
df_main["DNA repair defect"] = "No"

# select cols from df_cn 
df = df_cn.iloc[:, 4:].copy() # extract cols that have CNVs for genes
df = df.iloc[0:len(df_main), 0:len(df_main.columns)]
colnames = df.columns
df = pd.DataFrame(np.zeros((len(df), len(df.columns)))) # create a 0 filled df
df.columns = colnames

df = pd.concat([df_main.iloc[:, 0:4], df], axis = 1)
df_cn = pd.concat([df_cn, df])

# ADD PATIENTS WITH 0 ctDNA PERCENTAGE to DF_MUTS
#######################
row = pd.DataFrame({"Sample": ["CAVK-0002_Baseline"], "Gene": ["TP53"], "Effect": ["Germline missense mutation"]})
df_muts = pd.concat([df_muts, row]).reset_index(drop = True)
